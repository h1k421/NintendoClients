...
class CodeGenerator:
	...
	def generate_protocol(self, stream, proto):
		...
		stream.write_line("def __init__(self):")
		stream.indent()
		stream.write_line("self.request_decodes = {")
		for method in proto.methods.values():
			stream.write_line("\tself.METHOD_%s: self.request_decode_%s," %(method.name.upper(), method.name))
		stream.write_line("}")

		stream.write_line("self.response_decodes = {")
		for method in proto.methods.values():
			stream.write_line("\tself.METHOD_%s: self.response_decode_%s," %(method.name.upper(), method.name))
		stream.write_line("}")
		...
		for method in proto.methods.values():
			...
			stream.write_line("@staticmethod")
			stream.write_line("def request_decode_%s(input):" % method.name)
			stream.indent()
			stream.write_line("result = {}")
			stream.write_line()

			for param in method.request.vars:
				stream.write_line("result[\"%s\"] = %s" % (param.name, self.make_extract(param.type, "input")))
			...
			stream.write_line("@staticmethod")
			stream.write_line("def response_decode_%s(input):" % method.name)
			stream.indent()
			stream.write_line("result = {}")
			stream.write_line()

			for param in method.response.vars:
				stream.write_line("result[\"%s\"] = %s" % (param.name, self.make_extract(param.type, "input")))
